...
/**
 * A TOTP Generator, all credits given to the original
 * author
 * 
 * @Author Maxim Van de Wynckel on 01-Dec-16.
 */
public class TOTPGenerator {
    ...
    public static String generateTOTP(String key, int returnDigits) throws GeneralSecurityException {
        ...
        while (steps.length() < 16)
            steps.insert(0, "0");
        return TOTPGenerator.generateTOTP(key, steps.toString().toLowerCase(), returnDigits);
    }
    ...
    public static String generateTOTP256(String key, int returnDigits) throws GeneralSecurityException {
        ...
        while (steps.length() < 16)
            steps = "0" + steps;
        return TOTPGenerator.generateTOTP256(key, steps, returnDigits);
    }
    ...
    public static String generateTOTP512(String key, int returnDigits) throws GeneralSecurityException {
        ...
        while (steps.length() < 16)
            steps.insert(0, "0");
        return TOTPGenerator.generateTOTP512(key, steps.toString(), returnDigits);
    }
    ...
    public static boolean validate(String submittedOTP, byte[] secret, int numDigits) throws GeneralSecurityException {
        ...
        String generatedTOTP = TOTPGenerator.generateTOTP(new String(secret), numDigits);
        ...
        if (!result) {
            ...
            long T = (timeInMilis - TOTPGenerator.TIME_ZERO) / TOTPGenerator.TIME_SLICE_X;
            steps = new StringBuilder(Long.toHexString(T).toUpperCase());
            ...
            generatedTOTP = TOTPGenerator.generateTOTP(new String(secret), "" + steps, numDigits);
    ...
    public static boolean validate256(String submittedOTP, byte[] secret, int numDigits) throws GeneralSecurityException {
        ...
        String generatedTOTP = TOTPGenerator.generateTOTP256(new String(secret), numDigits);
        boolean result = generatedTOTP.equals(submittedOTP);
        ...
        if (!result) {
            ...
            generatedTOTP = TOTPGenerator.generateTOTP256(new String(secret), "" + timeInMilis, numDigits);
            result = generatedTOTP.equals(submittedOTP);
        }
    ...
    public static boolean validate512(String submittedOTP, byte[] secret, int numDigits) throws GeneralSecurityException {
        ...
        String generatedTOTP = TOTPGenerator.generateTOTP512(new String(secret), numDigits);
        boolean result = generatedTOTP.equals(submittedOTP);
        ...
        if (!result) {
            ...
            generatedTOTP = TOTPGenerator.generateTOTP512(new String(secret), "" + timeInMilis, numDigits);
            result = generatedTOTP.equals(submittedOTP);
        }
